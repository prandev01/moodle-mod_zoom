...
/**
 * Contains class mod_zoom2\privacy\provider
 *
 * @package    mod_zoom2
...
class provider implements
    ...
    public static function get_metadata(\core_privacy\local\metadata\collection $coll): \core_privacy\local\metadata\collection {
        ...
        $coll->add_database_table('zoom2_meeting_participants', [
            'name' => 'privacy:metadata:zoom2_meeting_participants:name',
            'user_email' => 'privacy:metadata:zoom2_meeting_participants:user_email',
            'join_time' => 'privacy:metadata:zoom2_meeting_participants:join_time',
            'leave_time' => 'privacy:metadata:zoom2_meeting_participants:leave_time',
            'duration' => 'privacy:metadata:zoom2_meeting_participants:duration',
        ], 'privacy:metadata:zoom2_meeting_participants');

        $coll->add_database_table('zoom2_meeting_details',
        ...
        $coll->add_database_table('zoom2_breakout_participants',
            ['userid' => 'privacy:metadata:zoom2_breakout_participants:userid'],
            'privacy:metadata:zoom2_breakout_participants');
    ...
    public static function get_users_in_context(\core_privacy\local\request\userlist $userlist) {
        ...
        $sql = "SELECT zmp.userid
                  ...
                  JOIN {zoom2_meeting_details} zmd ON zmd.id = zmp.detailsid
                  JOIN {zoom2} z ON zmd.zoom2id = z.id
        ...
        $sql = "SELECT zbp.userid
                  FROM {zoom2_breakout_participants} zbp
                  JOIN {zoom2_meeting_breakout_rooms} zmbr ON zbp.breakoutroomid = zmbr.id
                  JOIN {zoom2} z ON zmbr.zoom2id = z.id
                  JOIN {modules} m ON m.name = :modulename
                  JOIN {course_modules} cm ON z.id = cm.instance AND m.id = cm.module
                 WHERE cm.id = :instanceid";
    ...
    public static function export_user_data(\core_privacy\local\request\approved_contextlist $contextlist) {
        ...
        $sql = "SELECT zmp.id,
            ...
            INNER JOIN {modules} m ON m.id = cm.module AND m.name = :modname
            INNER JOIN {zoom2} z ON z.id = cm.instance
            INNER JOIN {zoom2_meeting_details} zmd ON zmd.zoom2id = z.id
            INNER JOIN {zoom2_meeting_participants} zmp ON zmp.detailsid = zmd.id
    ...
    public static function delete_data_for_all_users_in_context(\context $context) {
        ...
        if ($cm = get_coursemodule_from_id('zoom2', $context->instanceid)) {
            $meetingdetails = $DB->get_records('zoom2_meeting_details', ['zoom2id' => $cm->instance]);
            foreach ($meetingdetails as $meetingdetail) {
                $DB->delete_records('zoom2_meeting_participants', ['detailsid' => $meetingdetail->id]);
            }

            $DB->delete_records('zoom2_meeting_details', ['zoom2id' => $cm->instance]);
            ...
            foreach ($breakoutrooms as $room) {
                $DB->delete_records('zoom2_breakout_participants', ['breakoutroomid' => $room->id]);
            }

            $DB->delete_records('zoom2_meeting_breakout_rooms', ['zoom2id' => $cm->instance]);
        }
    }
    ...
    public static function delete_data_for_user(\core_privacy\local\request\approved_contextlist $contextlist) {
        ...
        foreach ($contextlist->get_contexts() as $context) {
            ...
            if ($cm = get_coursemodule_from_id('zoom2', $context->instanceid)) {
                $meetingdetails = $DB->get_records('zoom2_meeting_details', ['zoom2id' => $cm->instance]);
                foreach ($meetingdetails as $meetingdetail) {
                    $DB->delete_records('zoom2_meeting_participants', ['detailsid' => $meetingdetail->id, 'userid' => $user->id]);
                }
                ...
                $breakoutrooms = $DB->get_records('zoom2_meeting_breakout_rooms', ['zoom2id' => $cm->instance]);
                foreach ($breakoutrooms as $room) {
                    $DB->delete_records('zoom2_breakout_participants', ['breakoutroomid' => $room->id, 'userid' => $user->id]);
                }
            }
        }
    }
    ...
    public static function delete_data_for_users(\core_privacy\local\request\approved_userlist $userlist) {
        ...
        $sql = "SELECT zmd.id
                  ...
                  JOIN {zoom2} z ON zmd.zoom2id = z.id
                  JOIN {modules} m ON m.name = 'zoom2'
                  JOIN {course_modules} cm ON z.id = cm.instance AND m.id = cm.module
                  JOIN {context} ctx ON ctx.instanceid = cm.id AND ctx.contextlevel = :modlevel
                 WHERE ctx.id = :contextid";

        $DB->delete_records_select('zoom2_meeting_participants', "userid $insql AND detailsid IN ($sql)", $params);
        ...
        $DB->delete_records_select('zoom2_breakout_participants', "userid $insql AND breakoutroomid IN ($sql)", $params);
    }
}
