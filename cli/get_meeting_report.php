...
Example:
\$sudo -u www-data /usr/bin/php mod/zoom2/cli/get_meeting_report.php --start=2020-03-31 --end=2020-04-01
";
    cli_error($help);
}
...
if (!empty($options['hostuuid'])) {
    ...
    $hostuuids = $DB->get_fieldset_select('zoom2', 'DISTINCT host_id', 'course=:courseid', ['courseid' => $options['courseid']]);
    if (empty($hostuuids)) {
        cli_writeln(get_string('nozoom2sfound', 'mod_zoom2'));
...
$meetingtask = new mod_zoom2\task\get_meeting_reports();
