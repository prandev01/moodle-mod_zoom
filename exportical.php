...
/**
 * Export ical file for a zoom2 meeting.
 *
 * @package    mod_zoom2
...
if ($config->showdownloadical == ZOOM2_DOWNLOADICAL_DISABLE) {
    $disabledredirecturl = new moodle_url('/mod/zoom2/view.php', ['id' => $id]);
    throw new moodle_exception('err_downloadicaldisabled', 'mod_zoom2', $disabledredirecturl);
}
...
if ($zoom2->recurring && $zoom2->recurrence_type == ZOOM2_RECURRINGTYPE_NOTIME) {
    $errorredirecturl = new moodle_url('/mod/zoom2/view.php', ['id' => $id]);
    throw new moodle_exception('err_downloadicalrecurringnofixed', 'mod_zoom2', $errorredirecturl);
}
...
$convertedtext = html_to_text($zoom2->intro);
$descriptiontext = get_string('calendardescriptionURL', 'mod_zoom2', $CFG->wwwroot . '/mod/zoom2/view.php?id=' . $cm->id);
if (!empty($convertedtext)) {
    $descriptiontext .= get_string('calendardescriptionintro', 'mod_zoom2', $convertedtext);
}
...
$params = ['modulename' => 'zoom2', 'instance' => $zoom2->id];
...
if (empty($events)) {
    ...
    $errorredirecturl = new moodle_url('/mod/zoom2/view.php', ['id' => $id]);
    throw new moodle_exception('err_downloadicalrecurringempty', 'mod_zoom2', $errorredirecturl);
}

// Iterate over all events.
// We will add each event as an individual iCal event.
foreach ($events as $event) {
    $icalevent = zoom2_helper_icalendar_event($event, $descriptiontext);
