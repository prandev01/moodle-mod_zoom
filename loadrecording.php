...
$view = $DB->get_record('zoom2_meeting_recordings_view', $params);
if (!empty($view)) {
    if (empty($view->viewed)) {
        $view->viewed = 1;
        $view->timemodified = $now;
        $DB->update_record('zoom2_meeting_recordings_view', $view);
    }
} else {
    $view = new stdClass();
    ...
    $view->viewed = 1;
    $view->timemodified = $now;
    $view->id = $DB->insert_record('zoom2_meeting_recordings_view', $view);
}
